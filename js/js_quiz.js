...
import { js_ques } from "./ques/js_ques.js";
let username;
let empty = false;
...
function Auth() {
    ...
    let incorrect_ans = 0;
    let percent_score = 0;
    let not_attempt = js_ques.length;

    console.log("Start now ****not_attempt === ", not_attempt);

    console.log("HTML questiuons === ", js_ques[0]["opt"]);
    console.log("HTML questiuons === ", js_ques[0]["opt"][1]);

    let banner_heading = document.getElementById("banner_heading");
    banner_heading.style.fontSize = "40px";
    banner_heading.innerHTML = "JS Online Quiz";
    ...
    // console.log("signout_button ===== ", signout_button);
    ...
    function ques_submit(actual_ans, parent_ele, sel_ans) {
        ...
        if (sel_ans !== null) {
            ...
            if (sel_ans == actual_ans) {
                ...
                score += 10;
                localStorage.setItem("js_correct_answer",score);
    ...
    finish_btn.onclick = () => {
        ...
        let color = 'green';
        console.log("finish");
        score = JSON.parse(localStorage.getItem("js_correct_answer"));
        console.log("final SCore == ", score);
        check_not_attempt();
        // not_attempt = 0;

        percent_score = (score / (js_ques.length * 10)) * 100;
        // percent_score = 50;
        ...
        <div class="main">
            ...
            setTimeout(() => {
                ...
                result.appendChild(create_div('h2' , 'result_heading', 'result_heading', 'Online JS Quiz'));
                ...
                document.querySelector('.inner_text').appendChild((create_div('h6', 'inner_text', 'inner_text', `Number of Questions#  ${js_ques.length}`)));
                ...
                inner_ele.appendChild(create_div('td', 'td', 'td', `${js_ques.length}`));
    ...
    function check_not_attempt() {
        ...
        // not_attempt = 0;
        let div_id;
        console.log("js_ques.length ====== ", js_ques.length);

        for (ind; ind < js_ques.length; ind++) {
            div_id = 1 + ind;
